...
# ------------------------------------------------------------------------------
converters = {f"{i/100:.2f}": mp.mpf for i in range(25, 26)} # We should change the range in the future
...
def lambda_marvizi_melrose(j, str_e, e):
    ...
    if j % 2 == 1:
        return mpf(0)
    q = j
    mmc = (mpf(q)**2) * T_of_q_j(q, j, str_e, e)
    accord = mpf('0.01') # HOW TO CHANGE THIS???
    while True:
        q += 1
        mmc_new = (mpf(q)**2) * T_of_q_j(q, j, str_e, e)
...
def test_lambda_marvizi_melrose(j, str_e, e):
    ...
    q_trivial = 999
    start = time.time()
    mmc_trivial = (mpf(q_trivial)**2) * T_of_q_j(q_trivial, j, str_e, e)
    elapsed_trivial = time.time() - start
    print(mmc_list)
    # For plotting, convert mpmath numbers to floats
    plt.plot([float(x) for x in mmc_list[-800:]])
    plt.show()
...
def reduced_T_qj_matrix(max_q, max_j, str_e, e, lambda_MM):
    ...
    for q in range(1, max_q + 1):
        ...
        for j in range(1, max_j + 1):
            ...
            reduced_matrix[q - 1, j - 1] = T_qj - kappa_j / (mpf(q)**2)
...
max_q, max_j = 70, 70  # Dimensions of the reduced T matrix
